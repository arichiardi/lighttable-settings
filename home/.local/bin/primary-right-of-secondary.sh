...
if [ -z "$secondary_name" ]; then
    echo "Failed: cannot find secondary monitors."
    exit 1
fi
...
secondary_width=$(echo "$secondary_resolution" | awk -F 'x' '{print $1}')
secondary_height=$(echo "$secondary_resolution" | awk -F 'x' '{print $2}')

secondary_position=0x0
if [[ -z $primary_position ]]; then
    primary_position="$secondary_width"x0
fi
...
--output $primary_name --mode $primary_resolution --scale $primary_scale --pos $primary_position --primary \
--output ${secondary_array[0]} --mode $secondary_resolution --scale $secondary_scale --pos $secondary_position \
--output VIRTUAL1 --off"
